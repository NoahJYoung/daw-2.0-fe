...
export const SampleEditor = () => {
  ...
  >({});
  const { saveSamplePack, getSamplePackById } = useFileSystem();
  const { toast } = useToast();
  const { samplePackId } = useParams({ strict: false });
  const navigate = useNavigate();

  const handleSave = async () => {
    try {
      const zip = await createSamplesZip(
        packName,
        packDescription,
        samples,
        coverImage
      ...
      if (zip) {
        const createdId = await saveSamplePack(packName, zip, samplePackId);
        if (createdId) {
          navigate({ to: `/app/samples/${createdId}` });
          ...
          toast({
            title: "Success!",
            description: "Sample pack saved successfully",
          });
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    const loadSamplePackData = async () => {
      if (samplePackId) {
        const samplePack = getSamplePackById(samplePackId);
        if (samplePack) {
          setPackDescription(samplePack.description);
          setPackName(samplePack.name);
          setLoading(true);
          try {
            const data = await unzipProjectFile(samplePack.data);
            const loadedSamples: Record<string, { file: File; name: string }> =
              {};

            for (const key of Object.keys(data)) {
              if (key !== "cover.png") {
                const sampleFile = new File([data[key]], key);
                const buffer = await blobToAudioBuffer(sampleFile);
                sampler.add(key as Tone.Unit.Note, buffer);
                loadedSamples[key] = {
                  file: sampleFile,
                  name: key,
                };
              } else {
                const coverFile = new File([data[key]], "Cover.png");
            ...
            toast({
              variant: "destructive",
              title: "Oops!",
              description: (error as Error).message ?? "Error loading samples",
            });
          } finally {
            setLoading(false);
          }
  ...
  return (
    <>
      <AppLoader loading={loading} text="Loading Samples..." />
